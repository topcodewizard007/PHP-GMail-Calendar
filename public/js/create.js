...
$("#create-update-event").on("click", function (e) {
  ...
  if ($("#event-type").val() == "FIXED-TIME") {
    ...
    if (
      $("#event-end-time").datetimepicker("getValue") <
      $("#event-start-time").datetimepicker("getValue")
    ) {
  ...
  parameters = {
    ...
    start_time:
      $("#event-type").val() == "FIXED-TIME"
        ? $("#event-start-time").val().replace(" ", "T") + ":00"
        : null,
    end_time:
      $("#event-type").val() == "FIXED-TIME"
        ? $("#event-end-time").val().replace(" ", "T") + ":00"
        : null,
    event_date:
      $("#event-type").val() == "ALL-DAY" ? $("#event-date").val() : null,
    all_day: $("#event-type").val() == "ALL-DAY" ? 1 : 0,
    operation: $(this).attr("data-operation"),
    event_id:
      $(this).attr("data-operation") == "create"
        ? null
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (parameters.operation == "create") {
        ...
        $("#create-update-event").removeClass("loading");
        displaySnackbar(
          "Event with ID " + parameters.event_id + " was updated",
          "success",
          console.log("event upate success")
        );
      }
    },
...
$("#delete-event").on("click", function (e) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      displaySnackbar(
        "Event ID " + parameters.event_id + " deleted",
        "success",
        redirect()
      );
    },
